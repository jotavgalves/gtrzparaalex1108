...
export const voucherApi: VoucherApi = {
  async getState(): Promise<VoucherState> {
    const payload: unknown = await invoke(IPC_CHANNELS.vouchersGetState);
  ...
  async create(input: CreateVoucherInput): Promise<Voucher> {
    ...
    const payload: unknown = await invoke(IPC_CHANNELS.vouchersCreate, parsedInput);
