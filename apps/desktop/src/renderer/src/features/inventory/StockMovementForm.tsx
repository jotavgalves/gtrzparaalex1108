...
export function StockMovementForm({
  ...
  const nextQuantity = useMemo(() => {
    ...
    return intent === 'entry' ? product.quantity + parsedQuantity : product.quantity - parsedQuantity;
  ...
  async function handleSubmit(event: SyntheticEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (intent === 'decrease' && parsedQuantity > product.quantity) {
        throw new Error(`A baixa não pode ultrapassar o saldo atual de ${String(product.quantity)} unidades.`);
      }
  ...
  return (
    <form className="movement-form" onSubmit={(event) => void handleSubmit(event)}>
      ...
      <div className="movement-balance-preview">
        ...
        <strong className={nextQuantity < 0 ? 'text-danger' : undefined}>{Math.max(nextQuantity, 0)} un.</strong>
      </div>
      <div className="movement-form__grid">
        <label className="form-field">
          <span>Motivo</span>
          <select onChange={(event) => setType(event.target.value as StockMovementType)} value={type}>
            {movements.map((movement) => <option key={movement.value} value={movement.value}>{movement.label}</option>)}
          </select>
        </label>
        <label className="form-field">
          <span>Quantidade</span>
          <input max={intent === 'decrease' ? product.quantity : undefined} min="1" onChange={(event) => setQuantity(event.target.value)} required step="1" type="number" value={quantity} />
        </label>
      </div>
      {type === 'correction-negative' ? (
        <p className="form-hint">Correção negativa desfaz uma entrada lançada a maior e também reduz o aporte líquido calculado do produto.</p>
      ) : null}
      {type === 'loss' || type === 'breakage' ? (
        <p className="form-hint">Perda e quebra reduzem o valor que ainda existe no estoque, mas preservam o custo que realmente foi desembolsado.</p>
      ) : null}
      ...
      <div className="product-form__actions">
        <button className="button button--ghost" disabled={busy} onClick={onCancel} type="button"><X size={16} aria-hidden="true" />Cancelar</button>
        <button className="button button--primary" disabled={busy || (intent === 'decrease' && product.quantity === 0)} type="submit">
          {intent === 'entry' ? <ArrowDownToLine size={17} aria-hidden="true" /> : <ArrowUpFromLine size={17} aria-hidden="true" />}
